...
7. ~~**Vulnerability feed**~~ — shipped: Wordfence Intelligence v3 sync (free API key, `vuln:sync` + weekly schedule + "Sync now" on Settings) into a local `vulnerabilities` table (raw records kept); inventory is re-matched with version_compare semantics on every sync and every inventory refresh; per-item and per-site vulnerability badges. Patchstack remains an optional secondary source — their DB is free to browse but the API/monitoring is paid.
